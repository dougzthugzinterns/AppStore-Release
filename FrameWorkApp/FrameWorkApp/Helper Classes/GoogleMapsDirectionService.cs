...
namespace FrameWorkApp
{
	public class GoogleMapsDirectionService
	{
		...
		//PLEASE USE THIS before AND after decoding your paths
		public void resetLastCoordinateInPath(){
			coordinateInLastStep.Latitude = -500;
			coordinateInLastStep.Longitude = -500;
		}

		public Polyline constructPolyline (List<CLLocationCoordinate2D> decodedCoordinates){
		...
		public List<Google.Maps.Polyline> getPathsFromSingleCallout(System.Xml.XmlDocument xmlString){
			...
			foreach (System.Xml.XmlNode singleLeg in legs) {
				...
				foreach (System.Xml.XmlNode singleStepNode in stepNodes) {
					...
					Polyline currentPolyline = constructPolyline (decodedCoordinates);
		 ...
		 * */
		public List<CLLocationCoordinate2D> DecodePolylinePoints(string encodedPoints) 
		{
		...
		public String createGoogleDirectionServiceCalloutString(int startingIndexOfCoordinatesToPlotPathWith, List<CLLocationCoordinate2D> coordinatesToPlotPathWith)
		{
		...
		public  List<Google.Maps.Polyline> performGoogleDirectionServiceApiCallout(List<CLLocationCoordinate2D> coordinatesToPlotPathWith){
			resetLastCoordinateInPath ();
			...
			try
			{
				for(int i = 0; i <coordinatesToPlotPathWith.Count; i = i +9){
					if(i+1 < coordinatesToPlotPathWith.Count){
						...
						String requestURLwithParameters = createGoogleDirectionServiceCalloutString(i, coordinatesToPlotPathWith);
						...
						int numberOfCalloutAttempts = 0;
						while(wasCalloutSuccessful == false && numberOfCallouts < NUMBER_OF_CALLOUT_ATTEMPTS){ // if call fails, keep calling for 10 times
							//sleep 2 seconds ...
						...
						if(wasCalloutSuccessful == false){
							allPolylinesToShowOnMap.Clear();
							break;
						}
						numberOfCallouts++;
